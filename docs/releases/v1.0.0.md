...
contracts, and a deliberately slim packed package. The release is major
because direct `Workspace.closeCycle(...)` callers must now pass an
explicit `outcome`.
...
get a clearer close-cycle contract, and the packed artifact behaves more
